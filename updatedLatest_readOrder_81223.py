...
# pred = doctr_predictions('/home2/sreevatsa/test_images/LokSabha/11.jpg')
# df = pd.DataFrame(pred)
# print(df.T)
# d = json.load(open('/home2/sreevatsa/testtttt_uuiyiyiyiyi.json'))
# print(d)
# df = pd.read_json('/home2/sreevatsa/testtttt_uuiyiyiyiyi.json')
# print(df)

def save_TLBR_asCSV(df, new_df):
    top_left = []
    bottom_right = []
    id = []
    ...
    for i in range(0,len(df)):
        ...
        top_left_coos = [x1,y1]
        bottom_right_coos = [x2,y2]
        top_left.append(top_left_coos)
        bottom_right.append(bottom_right_coos)
        id.append(i)
    new_df['Top-Left'] = top_left
    new_df['Bottom-Right'] = bottom_right
    new_df['Id'] = id
...
#UPDATED FUNCTIONS; different versions of vectorized functions of the same to calculate neighboring boxes are saved in neigh_vectorized_funcs.py
...
# def return_visualise_paragraph_order(image, target_components, euclidean,component):
...
#     # output_path = 'paragraph_order.png'
#     # cv2.imwrite(output_path, cv2.cvtColor(image_with_boxes, cv2.COLOR_RGB2BGR))
#     output = cv2.cvtColor(image_with_boxes, cv2.COLOR_RGB2BGR)
#     return output
...
# def Reading_Order_Generator(image_file, inputt, width_p, header_p, footer_p):
def Reading_Order_Generator(image_file, width_p, header_p, footer_p):

    
    logtime(t,'getting doctr preds')
    pred = doctr_predictions(image_file)
    logtime(t,'got doctr preds')
    # print(len(pred[0]))
    df = pd.DataFrame(pred)
    ...
    # kde = pd.DataFrame()
    logtime(t,'calculating center points')
    # smai = pd.DataFrame() #added for SMAI
    # save_TLBR_asCSV(df,smai) #added for SMAI
    
    calculate_center_points(df, euclidean) #changing kde to euclidean
    # print(euclidean)
    # calculate_center_points(df,smai) #added for SMAI
    # smai.to_csv('TLBR_{}.csv'.format(inputt)) #added for SMAI
    logtime(t,'finding closest neighbors')
    horizontal_neighbors, vertical_neighbors = find_closest_neighbors(euclidean) #changing kde to euclidean
    logtime(t,'calculating kde estimates')
    # print(horizontal_neighbors)
    # print("  ###############    ")
    # print(vertical_neighbors)
    ...
    # ax[0].plot(horizontal_neighbors,kkd)
    # ax[1].plot(y,kdk)
    # plt.savefig('ana.png')
    # plt.plot(sorted(horizontal_neighbors))
    # plt.savefig('jhj.png')
    # print(peak)
    # calculate_center_points(df,euclidean)
    logtime(t,'finding neighbor boxes in all direcs')
    ...
    #optimized functions
    # print("############################################################################################")
    # calculate_rightbox_opti2(euclidean,x)
    # calculate_leftbox_opti2(euclidean,x)
    # calculate_topbox_opti2(euclidean,y)
    # calculate_bottombox_opti2(euclidean,y)
    
    logtime(t,'got neighbor boxes')
    
    # cv2.imwrite('ngboxes_connected.png', make_connections(cv2.imread(image_file),euclidean)) #for visualizing connections

    # euclidean.to_csv("connections_29.csv")       #commented - to avoid saving euclidean as csv 
    # euclidean = pd.read_csv("connections.csv") #commented - to avoid saving euclidean as csv 
    logtime(t,'create graph')
    ...
    image = cv2.imread(image_file)
    logtime(t,'recognising paras')
    component = recognise_paragraphs(image, target_components, euclidean, image_file, width_p, header_p, footer_p)
    print(component)
    logtime(t,'finding min dist')
    min_idx =  minimum_euclidean(component)
    # print(min_idx)
    logtime(t,'get paragraph order')
    component = paragraph_order(component)
    print(component)
    # print(component_p,file=open("component_p.txt","a"))
    logtime(t,'get column order')
    # component_col = column_order(component)
    # print(component_col)
    # merge_columns(image,component_col)
    # get_column_order(image,component)
    if args.col_only is True and args.para_only is True:
        logtime(t,'visualise paragraph and column order')
        get_col(image,component)
        visualise_paragraph_order(image, target_components, euclidean,component)
    if args.col_only is True and args.para_only is False:
        get_col(image,component)
    if args.para_only is True and args.col_only is False:
        logtime(t,'visualise paragraph order')
        visualise_paragraph_order(image, target_components, euclidean,component)
       # return_visualise_paragraph_order(image, target_components, euclidean,component)

    elif args.para_only is False and args.col_only is False:
        new = pd.DataFrame()
        sort_order = component.sort_values('Order').index
        new = component.loc[sort_order]
        new = new.reset_index(drop=True)
        new_euclidean = pd.DataFrame()
        logtime(t,'get word order')
...
inputt = 68
print(inputt)
# Reading_Order_Generator('/home2/sreevatsa/test_images/{}.jpg'.format(inputt), inputt, 0, 10, 5)

# for inputt in [29,68,101,145,152,159,174,201,232,250]:
# for inputt in [159]:
#     Reading_Order_Generator('/home2/sreevatsa/test_images/{}.jpg'.format(inputt), inputt)

Reading_Order_Generator(args.ImageFile, 0, 10, 5)
